...
def integrate_events_by_fixed_frames_number(events: Dict, split_by: str, frames_num: int, H: int, W: int) -> np.ndarray:
    ...
    '''
    t, x, y, p = (events[key] for key in ('t', 'x', 'y', 'p'))  # get events data
    j_l, j_r = cal_fixed_frames_number_segment_index(t, split_by, frames_num)   # calculate the segment index
    frames = np.zeros([frames_num, 2, H, W])    # initialize frames
    for i in range(frames_num):    # integrate events to frames
        frames[i] = integrate_events_segment_to_frame(x, y, p, H, W, j_l[i], j_r[i])    # integrate events to frames
...
def integrate_events_file_to_frames_file_by_fixed_frames_number(loader: Callable, events_np_file: str, output_dir: str, split_by: str, frames_num: int, H: int, W: int, print_save: bool = False) -> None:
    ...
    '''
    fname = os.path.join(output_dir, os.path.basename(events_np_file))  # save frames in the same directory as the events file
    np_savez(fname, frames=integrate_events_by_fixed_frames_number(loader(events_np_file), split_by, frames_num, H, W))     # save frames
    if print_save:  # print saved files' paths
        print(f'Frames [{fname}] saved.')   # print saved files' paths
...
class NeuromorphicDatasetFolder(DatasetFolder): #   torch.utils.data.Dataset
    def __init__(
            self,
            root: str,
            train: bool = None,
        ...
        events_np_root = os.path.join(root, 'events_np')    # the directory for saving npz files of events

        if not os.path.exists(events_np_root):  # if the directory does not exist, create it
