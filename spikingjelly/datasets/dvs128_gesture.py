from typing import Callable, Dict, Optional, Tuple  #引入模块
...
class DVS128Gesture(sjds.NeuromorphicDatasetFolder):    #定义DVS128Gesture类，继承自sjds.NeuromorphicDatasetFolder类。
    def __init__(   #定义初始化方法。
            self,
            root: str,
            train: bool = None,
            data_type: str = 'event',
            frames_number: int = None,  #帧数。
            split_by: str = None,   #分割方式。
            duration: int = None,   #持续时间。
            custom_integrate_function: Callable = None,   #自定义集成函数。
            custom_integrated_frames_dir_name: str = None,  #自定义集成帧目录名称。
            transform: Optional[Callable] = None,   #可选的转换。
            target_transform: Optional[Callable] = None,    #可选的目标转换。
    ) -> None:  #返回值为None。
        """
        ...
        """ 
        assert train is not None    #断言train不为空。
        super().__init__(root, train, data_type, frames_number, split_by, duration, custom_integrate_function, custom_integrated_frames_dir_name, transform, target_transform)  #调用父类的初始化方法。
    @staticmethod   #装饰器，静态方法。
    def resource_url_md5() -> list:   #定义resource_url_md5方法，返回值为list。
        '''
    ...
    @staticmethod
    def downloadable() -> bool:   #定义downloadable方法，返回值为bool。
        '''
    ...
    @staticmethod
    def extract_downloaded_files(download_root: str, extract_root: str):    #定义extract_downloaded_files方法。
        '''
    ...
    @staticmethod
    def load_origin_data(file_name: str) -> Dict:   #定义load_origin_data方法，返回值为Dict。
        '''
    ...
    @staticmethod
    def split_aedat_files_to_np(fname: str, aedat_file: str, csv_file: str, output_dir: str):   #定义split_aedat_files_to_np方法。
        events = DVS128Gesture.load_origin_data(aedat_file)  #调用load_origin_data方法。
        ...
        csv_data = np.loadtxt(csv_file, dtype=np.uint32, delimiter=',', skiprows=1) #读取csv文件。

        # Note that there are some files that many samples have the same label, e.g., user26_fluorescent_labels.csv
        label_file_num = [0] * 11   #初始化label_file_num。
        ...
        for i in range(csv_data.shape[0]):  #遍历csv_data。
            # the label of DVS128 Gesture is 1, 2, ..., 11. We set 0 as the first label, rather than 1
            label = csv_data[i][0] - 1  #标签。
            t_start = csv_data[i][1]        #开始时间。
            t_end = csv_data[i][2]    #结束时间。
            mask = np.logical_and(events['t'] >= t_start, events['t'] < t_end)  #掩码。
            file_name = os.path.join(output_dir, str(label), f'{fname}_{label_file_num[label]}.npz')    #文件名。
            np_savez(file_name, #保存文件。
                     t=events['t'][mask],
                     x=events['x'][mask],
                     y=events['y'][mask],
                     p=events['p'][mask]
            ...
            label_file_num[label] += 1  #标签文件数加1。
    ...
    @staticmethod
    def create_events_np_files(extract_root: str, events_np_root: str):   #定义create_events_np_files方法。
        '''
        :param extract_root: Root directory path which saves extracted files from downloaded files
        :type extract_root: str
        ...
        '''
        aedat_dir = os.path.join(extract_root, 'DvsGesture')    #aedat_dir。
        train_dir = os.path.join(events_np_root, 'train')   #train_dir。
        test_dir = os.path.join(events_np_root, 'test') #test_dir。
        os.mkdir(train_dir) #创建train_dir。
        os.mkdir(test_dir)  #创建test_dir。
        print(f'Mkdir [{train_dir, test_dir}.')
        for label in range(11): #遍历11个标签。
            os.mkdir(os.path.join(train_dir, str(label)))   #创建train_dir。
            os.mkdir(os.path.join(test_dir, str(label)))    #创建test_dir。
        print(f'Mkdir {os.listdir(train_dir)} in [{train_dir}] and {os.listdir(test_dir)} in [{test_dir}].')   

        with open(os.path.join(aedat_dir, 'trials_to_train.txt')) as trials_to_train_txt, open(   #打开trials_to_train.txt。
                os.path.join(aedat_dir, 'trials_to_test.txt')) as trials_to_test_txt:   #打开trials_to_test.txt。
            # use multi-thread to accelerate
            t_ckp = time.time() #记录时间。
            with ThreadPoolExecutor(max_workers=min(multiprocessing.cpu_count(), configure.max_threads_number_for_datasets_preprocess)) as tpe:  #创建线程池。
                sub_threads = []    #子线程。
                print(f'Start the ThreadPoolExecutor with max workers = [{tpe._max_workers}].')   #打印信息。


                for fname in trials_to_train_txt.readlines():   #遍历trials_to_train_txt。
                    fname = fname.strip()   #去掉空格。
                    if fname.__len__() > 0: #如果长度大于0。
                        aedat_file = os.path.join(aedat_dir, fname)   #aedat_file。
                        fname = os.path.splitext(fname)[0]  #去掉扩展名。
                        sub_threads.append(tpe.submit(DVS128Gesture.split_aedat_files_to_np, fname, aedat_file, os.path.join(aedat_dir, fname + '_labels.csv'), train_dir)) #添加子线程。


                for fname in trials_to_test_txt.readlines():    #遍历trials_to_test_txt。
                    fname = fname.strip()   
                    if fname.__len__() > 0:
                        aedat_file = os.path.join(aedat_dir, fname) #aedat_file。
                        fname = os.path.splitext(fname)[0]  #去掉扩展名。
                        sub_threads.append(tpe.submit(DVS128Gesture.split_aedat_files_to_np, fname, aedat_file, 
                                   os.path.join(aedat_dir, fname + '_labels.csv'), test_dir))   #添加子线程。


                for sub_thread in sub_threads:  #遍历子线程。
                    if sub_thread.exception():  #如果有异常。
